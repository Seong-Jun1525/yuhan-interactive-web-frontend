...
const DeptMap = () => {
  ...
  return (
    <group>
      <DeptFloor position={[0, -11, 0]} groundMapName={groundMapName} onMove={handleMove} />
      ...
      <React.Fragment>
        ...
        {groundMapName === "computer_sw_map" && <CSDeptHeadCharacter groundMapName={groundMapName} position={[-100, 0.7, -127]} scale={0.7} />}
        {groundMapName === "food_nutrition_map" && <FoodDeptHeadCharacter groundMapName={groundMapName} position={[0, 0.7, -100]} scale={0.7} />}
      ...
      <React.Fragment>
        {/* 캐릭터가 있는 경우에만 MainCharacter 컴포넌트 렌더링 */}
        {targetPosition && myChar !== "" && <MainCharacter myChar={myChar} position={targetPosition} />}
      </React.Fragment>
    </group>
  );
};
