...
export const useMainCharacter = ({ position, myChar }) => {
  ...
  useEffect(() => {
    ...
    if (charRef.current) {
      if (groundMapState.mapName === "yh_map") {
        charRef.current.position.set(myChar.currentPosition[0], myChar.currentPosition[1], myChar.currentPosition[2]);
      } else {
        if (charRef.current) {
          charRef.current.position.set(myChar.deptInitPosition[0], myChar.deptInitPosition[1], myChar.deptInitPosition[2]);
        }
      }
    }
  ...
  useEffect(() => {
    if (teleportValue) {
      ...
      charRef.current.position.set(teleportPosition[0], teleportPosition[1], teleportPosition[2]);
    }
  ...
  useEffect(() => {
    if (aerialViewValue) {
      if (kioskValue || busValue || smokingValue || statueValue || yuhanTvZoneCameraFlag.current || welcomeZoneCameraFlag.current) {
  ...
  useFrame(({ camera }) => {
    ...
    if (aerialViewValue) {
      ...
      if (groundMapState.mapName === "yh_map") {
        // Start Zone
        if (currentPosition.x <= 285 && currentPosition.x >= 275 && currentPosition.z >= -360 && currentPosition.z <= -350) {
          // console.log('test')
        ...
        // 학교 밖 정문 앞 쪽
        else if (currentPosition.x > 250 && currentPosition.x < 520 && currentPosition.z < -325 && currentPosition.z >= -560) {
          // console.log('test')
          handleCamera(550, 150, -550);
        }
        // 그 외 학교 밖
        else if (currentPosition.x > 250 && currentPosition.x < 520 && currentPosition.z < 396 && currentPosition.z >= -325) {
          // console.log('test')
          handleCamera(600, 150, 50);
          // Bus Zone
          if (currentPosition.x <= 322 && currentPosition.x >= 262 && currentPosition.z >= -183 && currentPosition.z <= -143) {
            if (!gsapCameraState) {
        ...
        if (currentPosition.x > 147 && currentPosition.x <= 224 && currentPosition.z >= -28 && currentPosition.z < 285) {
          ...
          // 학생 식당 영역에 들어왔을 때
          if (currentPosition.x >= 179 && currentPosition.x <= 202 && currentPosition.z >= 128 && currentPosition.z <= 168) {
            if (!gsapCameraState) {
              setGsapCameraState(true);
        ...
        // 테라스 안쪽 좌측
        if (currentPosition.z <= 213 && currentPosition.z >= 143 && currentPosition.x <= 0 && currentPosition.x >= -42) {
        ...
        if (currentPosition.x <= 150 && currentPosition.z > 120) {
          ...
          // 창조관 키오스크 + 이벤트
          if (currentPosition.x >= -175 && currentPosition.x <= -155 && currentPosition.z >= 244 && currentPosition.z <= 284) {
            if (!gsapCameraState) {
              setGsapCameraState(true);
          ...
          // 테라스
          else if (currentPosition.x >= -140 && currentPosition.x <= -100 && currentPosition.z >= 180 && currentPosition.z <= 220) {
            if (!gsapCameraState) {
              setGsapCameraState(true);
        ...
        if (currentPosition.z <= 120 && currentPosition.z > -130) {
          if (currentPosition.x < -128 && currentPosition.x > -270) {
            ...
            // 유일한기념관 키오스크 + 이벤트
            if (currentPosition.x >= -248 && currentPosition.x <= -228 && currentPosition.z >= 74 && currentPosition.z <= 114) {
              if (!gsapCameraState) {
                setGsapCameraState(true);
          ...
          if (currentPosition.x <= -270) {
            ...
            // 유재라관 키오스크 + 이벤트
            if (currentPosition.x <= -279 && currentPosition.x >= -319 && currentPosition.z <= -65 && currentPosition.z >= -85) {
              if (!gsapCameraState) {
                setGsapCameraState(true);
            ...
            // 유재라관
            else if (currentPosition.x >= -370 && currentPosition.x <= -320 && currentPosition.z >= -130 && currentPosition.z <= -90) {
              // console.log('test')
              if (!gsapCameraState) {
                setGsapCameraState(true);
        ...
        if (currentPosition.x < -128 && currentPosition.x > -270) {
          if (currentPosition.z < -60 && currentPosition.z > -270) {
            handleCamera(currentPosition.x + 0, currentPosition.y + 100, currentPosition.z + 180);
            // 자유관 키오스크 + 이벤트
            if (currentPosition.x >= -191 && currentPosition.x <= -161 && currentPosition.z <= -214 && currentPosition.z >= -244) {
              if (!gsapCameraState) {
                setGsapCameraState(true);
        ...
        if (currentPosition.x <= -270 && currentPosition.x > -527 && currentPosition.z <= -254 && currentPosition.z > -528) {
        ...
        if (currentPosition.z <= -102 && currentPosition.z >= -200) {
          if (currentPosition.x >= -69 && currentPosition.x <= 58) {
            ...
            // 평화관 정문 키오스크
            if (currentPosition.x <= 45 && currentPosition.x >= 5 && currentPosition.z <= -126 && currentPosition.z >= -146) {
              if (!gsapCameraState) {
                setGsapCameraState(true);
          ...
          if (currentPosition.x > 58 && currentPosition.x <= 190) {
            ...
            if (currentPosition.x <= 189 && currentPosition.x >= 159 && currentPosition.z <= -170 && currentPosition.z >= -210) {
              // 봉사관 키오스크
              if (!gsapCameraState) {
                setGsapCameraState(true);
        ...
        // 나눔의 숲 3, 4사분면
        if (currentPosition.x >= -69 && currentPosition.x <= 190 && currentPosition.z < -200 && currentPosition.z >= -320) {
          // console.log('test')
          handleCamera(currentPosition.x, currentPosition.y + 100, currentPosition.z + 50);

          // 흡연구역 이벤트 발생 지역
          if (currentPosition.x <= 58 && currentPosition.x >= 25 && currentPosition.z <= -240 && currentPosition.z >= -280) {
            if (!gsapCameraState) {
              setGsapCameraState(true);
          ...
          } else if (currentPosition.x >= 66 && currentPosition.x <= 126 && currentPosition.z >= -230 && currentPosition.z < -200) {
            if (!gsapCameraState) {
              setGsapCameraState(true);
        ...
        // Welcome Zone
        if (currentPosition.x <= -260 && currentPosition.x >= -286 && currentPosition.z <= -420 && currentPosition.z >= -450) {
        ...
        // 유한TV Zone
        if (currentPosition.x <= -108 && currentPosition.x >= -148 && currentPosition.z <= -410 && currentPosition.z >= -450) {
        ...
        // 학교입구
        if (currentPosition.x > -32 && currentPosition.x <= 250 && currentPosition.z < -325 && currentPosition.z >= -560) {
          handleCamera(currentPosition.x + 180, currentPosition.y + 200, currentPosition.z + 0);
          // 동상 Zone
          if (currentPosition.x >= 22 && currentPosition.x <= 62 && currentPosition.z >= -535 && currentPosition.z <= -495) {
        ...
        // 3, 4사분면
        if (currentPosition.x <= 250 && currentPosition.x >= -250 && currentPosition.z > 0 && currentPosition.z <= 250) {
        ...
        if (currentPosition.x > 0 && currentPosition.x <= 250 && currentPosition.z >= -250 && currentPosition.z <= 0) {
          ...
          // 학과특징
          if (currentPosition.x >= 127 && currentPosition.x <= 167 && currentPosition.z >= -160 && currentPosition.z <= -120) {
